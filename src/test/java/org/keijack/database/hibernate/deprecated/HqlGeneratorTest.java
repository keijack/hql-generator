...
public class HqlGeneratorTest {
	...
	public void testGetHqlBasic() {
		...
		try {
			...
			TestCase.assertEquals(
					"from org.keijack.database.hibernate.deprecated.HibernateEntity testModel",
					hql.getFrom());
			TestCase.assertEquals(
	...
	public void testSelectField() {
		...
		try {
			HqlAndParams hql = HqlGenerator.generateHql(call);
			TestCase.assertEquals(
					"select distinct t.id, t.parent from org.keijack.database.hibernate.deprecated.HibernateEntity t",
					hql.getFrom());
		} catch (Exception e) {
			e.printStackTrace();
			TestCase.fail();
	...
	public void testQueryListEmbed() {
		...
		HqlAndParams hql = HqlGenerator.generateHql(call);
		TestCase.assertEquals(
				"from org.keijack.database.hibernate.deprecated.HibernateEntity testModel where 1 = 1 and ((testModel.strValue) like ? or (testModel.strValue) like ?) ",
				hql.getHql());
